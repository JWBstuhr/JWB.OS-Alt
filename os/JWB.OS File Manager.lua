...
local titleBar = configSet[1] or {txt = colors.white,back = colors.gray}
local addressBar = configSet[2] or {txt = colors.white,back = colors.lightGray}
local itemWindo = configSet[3] or {txt = colors.white,back = colors.black}
local rcmList = configSet[4] or {txt = colors.white,back = colors.lightGray} -- rcm = Right Click Menu List
local rcmTitle = configSet[5] or {txt = colors.white,back = colors.gray}
local dialogTitle = configSet[6] or {txt = colors.white,back = colors.gray}
local dialogWindo = configSet[7] or {txt = colors.white,back = colors.lightGray}
local scrollCol = configSet[8] or {off = colors.gray, button = colors.gray,back = colors.lightGray}
 
local tIcons = configSet[9] or {
    back = {tCol = "lightGray",bCol = "gray",txt = " < "},
    disk = {tCol = "yellow",bCol = "gray",txt = "[*]"},
    audio = {tCol = "red",bCol = "gray",txt = "(o)"},
    folder = {tCol = "lightGray",bCol = "gray",txt = "[=]"},
...
local function InputBox(title)
    ...
    local function drawBox()
        printC(ofsX,ofsY,colors.black,colors.gray,string.rep(" ",boxW))
        printC(ofsX+1,ofsY,colors.black,colors.gray,(title or "User Input"))
                    ...
                    if event[2] == 1 then -- left mouse
                        ...
                        elseif selected.id == "file" then
                            if dialogBox("Run file ?",selected.n,{"yes","no"}) == 1 then
                                shell.run("fg "..stringPath()..fSlash..selected.n)
                            end
                        flag = true
                        end
